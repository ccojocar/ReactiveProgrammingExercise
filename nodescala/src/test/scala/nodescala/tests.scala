...
class NodeScalaSuite extends FunSuite {
  ...
  test("A future should complete with value 1") {
  ...
  test("A future should complete with exception") {
    val any = Future.any(List(Future { throw new Exception }, Future { 2 }, Future { throw new Exception }))
    try {
  ...
  test("When calling delay the future should complete after 100 millis") {
    val delay = Future.delay(100 millis)
    try {
      Await.result(delay, 110 millis)
    } catch {
  ...
  test("CancellationTokenSource should allow stopping the computation when using Future.run") {
    val p = Promise[String]()
    val s = Future.run()(
      ctx => async {
